...
int main()
{
	...
	LOG_INFO("starting vcpu on thread");
	...
	LOG_INFO("calling map_virt while vcpu is running");
	mem->map_virt(*space, 0xFFFF800000002000, 0x1000, prot_rw);
	LOG_INFO("map_virt returned (sync ok)");
	...
	LOG_INFO("new region readback: {:#x}", readback);

	cpu->stop();
	vcpu_thread.join();

	LOG_INFO("rcx at stop: {:#x}", cpu->reg<x86::rcx>());

	auto pa = mem->virt_to_phys(*space, 0xFFFF800000001000);
	LOG_INFO("VA 0xFFFF800000001000 -> PA {:#x}", pa.value_or(0));

	LOG_INFO("mmu test passed");
}
