...
static void handle_init_unicode_string(const std::shared_ptr<emulator_t>& emulator,
	emulator_t::address_type destination_address, emulator_t::address_type source_address)
{
	auto destination_object = emulator_object_t<UNICODE_STRING>::view_at(emulator, destination_address);

	UNICODE_STRING destination = { };

	destination.Buffer = reinterpret_cast<PWSTR>(source_address);

	if (source_address)
	{
		const auto source_string = kernel::read_guest_wstring(*emulator, source_address);
		...
		THREAD_LOG("RtlInitUnicodeString called (destination=0x{:X}, source='{}')",
			destination_address, util::narrow_wstring(source_string));
	}
	else
	{
		THREAD_LOG("RtlInitUnicodeString called (destination=0x{:X}, source=null)", destination_address);
	}

	destination_object.write(destination);
}

static void handle_duplicate_unicode_string(const std::shared_ptr<emulator_t>& emulator,
	std::uint32_t flags, emulator_t::address_type source_string_address,
	emulator_t::address_type destination_string_ptr)
{
	THREAD_LOG("RtlDuplicateUnicodeString called (string in=0x{:X})", source_string_address);
	...
	auto destination_string_object = emulator_object_t<UNICODE_STRING>::view_at(emulator, destination_string_ptr);

	std::uint16_t length = 0;
	std::uint16_t max_length = 0;
	emulator_t::address_type source_buffer_address = 0;

	if (source_string_address)
	{
		const auto source_string_object = emulator_object_t<UNICODE_STRING>::view_at(emulator, source_string_address);
...
static void handle_free_unicode_string(const std::shared_ptr<emulator_t>& emulator,
	emulator_t::address_type string_address)
{
	auto string_object = emulator_object_t<UNICODE_STRING>::view_at(emulator, string_address);
...
static void handle_wcslen(const std::shared_ptr<emulator_t>& emulator,
	emulator_t::address_type str_address)
{
	if (!str_address)
	{
	...
	const auto str = kernel::read_guest_wstring(*emulator, str_address);
...
static void handle_wcscpy_s(const std::shared_ptr<emulator_t>& emulator,
	emulator_t::address_type dst_address, std::uint64_t size_in_words,
	emulator_t::address_type src_address)
{
...
static void handle_stricmp(const std::shared_ptr<emulator_t>& emulator,
	emulator_t::address_type str1_address, emulator_t::address_type str2_address)
{
...
static void handle_strcmp(const std::shared_ptr<emulator_t>& emulator,
	emulator_t::address_type str1_address, emulator_t::address_type str2_address)
{
	const auto str1 = kernel::read_guest_string(*emulator, str1_address);
	const auto str2 = kernel::read_guest_string(*emulator, str2_address);
...
static void handle_strncmp(const std::shared_ptr<emulator_t>& emulator,
	emulator_t::address_type str1_address, emulator_t::address_type str2_address,
...
static void handle_strnicmp(const std::shared_ptr<emulator_t>& emulator,
	emulator_t::address_type str1_address, emulator_t::address_type str2_address,
	std::uint64_t max_count)
{
	if (!max_count)
...
static void handle_strncpy(const std::shared_ptr<emulator_t>& emulator,
	emulator_t::address_type dest_address, emulator_t::address_type src_address,
	std::uint64_t count)
{
...
static void handle_wcsncpy(const std::shared_ptr<emulator_t>& emulator,
	emulator_t::address_type dest_address, emulator_t::address_type src_address,
	std::uint64_t count)
{
	if (!count)
...
static void handle_strstr(const std::shared_ptr<emulator_t>& emulator,
	emulator_t::address_type str1_address, emulator_t::address_type str2_address)
{
	const auto str1 = kernel::read_guest_string(*emulator, str1_address);
	const auto str2 = kernel::read_guest_string(*emulator, str2_address);
...
static void handle_wcscat_s(const std::shared_ptr<emulator_t>& emulator,
	emulator_t::address_type dst_address, std::uint64_t size_in_words,
	emulator_t::address_type src_address)
{
	constexpr std::uint32_t einval = 22;
...
static void handle_tolower(const std::shared_ptr<emulator_t>& emulator,
	std::int32_t c)
{
...
static void handle_init_ansi_string(const std::shared_ptr<emulator_t>& emulator,
	emulator_t::address_type destination_address, emulator_t::address_type source_address)
{
...
static void handle_unicode_to_utf8n(const std::shared_ptr<emulator_t>& emulator,
	emulator_t::address_type utf8_dest, std::uint32_t utf8_max_bytes,
	emulator_t::address_type utf8_actual_bytes_ptr, emulator_t::address_type unicode_src,
	std::uint32_t unicode_byte_count)
{
	...
	if (wchar_count > 0)
	{
		emulator_err_t error = emulator->read_virtual_memory(unicode_src, source_str.data(), unicode_byte_count);
	...
	{
		emulator_err_t error = emulator->write_virtual_memory(utf8_actual_bytes_ptr, &actual_bytes, sizeof(actual_bytes));
	...
	if (utf8_dest && utf8_max_bytes > 0)
	{
		...
		emulator_err_t error = emulator->write_virtual_memory(utf8_dest, utf8_result.data(), copy_size);
...
static void handle_ansi_string_to_unicode_string(const std::shared_ptr<emulator_t>& emulator,
	emulator_t::address_type destination_address, emulator_t::address_type source_address,
	std::uint8_t allocate_destination)
{
	_ANSI_STRING source = { };
...
static void handle_multi_byte_to_unicode_n(const std::shared_ptr<emulator_t>& emulator,
	emulator_t::address_type unicode_string, std::uint32_t max_bytes,
	emulator_t::address_type bytes_out, emulator_t::address_type multibyte_string,
...
static void handle_compare_string(const std::shared_ptr<emulator_t>& emulator,
	emulator_t::address_type string1_address, emulator_t::address_type string2_address,
	std::uint8_t case_insensitive)
{
...
static void handle_get_default_code_page(const std::shared_ptr<emulator_t>& emulator,
	emulator_t::address_type ansi_code_page_ptr, emulator_t::address_type oem_code_page_ptr)
{
...
static void handle_compare_unicode_string(const std::shared_ptr<emulator_t>& emulator,
	emulator_t::address_type string1_address, emulator_t::address_type string2_address,
	std::uint8_t case_insensitive)
{
	UNICODE_STRING str1_header = { };
...
static void handle_prefix_unicode_string(const std::shared_ptr<emulator_t>& emulator,
	emulator_t::address_type string1_address, emulator_t::address_type string2_address,
	std::uint8_t case_insensitive)
{
...
static void handle_equal_unicode_string(const std::shared_ptr<emulator_t>& emulator,
	emulator_t::address_type string1_address, emulator_t::address_type string2_address,
	std::uint8_t case_insensitive)
{
	const auto us1 = emulator_object_t<UNICODE_STRING>::view_at(emulator, string1_address).read();
...
static void handle_towlower(const std::shared_ptr<emulator_t>& emulator,
	std::uint32_t c)
{
...
static void handle_towupper(const std::shared_ptr<emulator_t>& emulator,
	std::uint32_t c)
{
	auto result = static_cast<wchar_t>(c);
...
{
	redirect_handler<handle_init_unicode_string>(emulator, mapped_image, "RtlInitUnicodeString");
	redirect_handler<handle_duplicate_unicode_string>(emulator, mapped_image, "RtlDuplicateUnicodeString");
	redirect_handler<handle_free_unicode_string>(emulator, mapped_image, "RtlFreeUnicodeString");
	redirect_handler<handle_wcslen>(emulator, mapped_image, "wcslen");
	redirect_handler<handle_wcscpy_s>(emulator, mapped_image, "wcscpy_s");
	redirect_handler<handle_stricmp>(emulator, mapped_image, "_stricmp");
	redirect_handler<handle_strcmp>(emulator, mapped_image, "strcmp");
	redirect_handler<handle_strncmp>(emulator, mapped_image, "strncmp");
	redirect_handler<handle_strnicmp>(emulator, mapped_image, "_strnicmp");
	redirect_handler<handle_strncpy>(emulator, mapped_image, "strncpy");
	redirect_handler<handle_wcsncpy>(emulator, mapped_image, "wcsncpy");
	redirect_handler<handle_strstr>(emulator, mapped_image, "strstr");
	redirect_handler<handle_wcscat_s>(emulator, mapped_image, "wcscat_s");
	redirect_handler<handle_tolower>(emulator, mapped_image, "tolower");
	redirect_handler<handle_init_ansi_string>(emulator, mapped_image, "RtlInitAnsiString");
	redirect_handler<handle_unicode_to_utf8n>(emulator, mapped_image, "RtlUnicodeToUTF8N");
	redirect_handler<handle_ansi_string_to_unicode_string>(emulator, mapped_image, "RtlAnsiStringToUnicodeString");
	redirect_handler<handle_multi_byte_to_unicode_n>(emulator, mapped_image, "RtlMultiByteToUnicodeN");
	redirect_handler<handle_compare_string>(emulator, mapped_image, "RtlCompareString");
	redirect_handler<handle_get_default_code_page>(emulator, mapped_image, "RtlGetDefaultCodePage");
	redirect_handler<handle_compare_unicode_string>(emulator, mapped_image, "RtlCompareUnicodeString");
	redirect_handler<handle_prefix_unicode_string>(emulator, mapped_image, "RtlPrefixUnicodeString");
	redirect_handler<handle_equal_unicode_string>(emulator, mapped_image, "RtlEqualUnicodeString");
	redirect_handler<handle_towlower>(emulator, mapped_image, "towlower");
	redirect_handler<handle_towupper>(emulator, mapped_image, "towupper");
}
