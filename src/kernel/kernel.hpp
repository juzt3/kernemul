...
namespace kernel
{
	...
	inline std::vector<std::shared_ptr<mapped_image_t>> module_entries;
	inline std::shared_ptr<mapped_image_t> emulated_module;
	...
	[[nodiscard]] std::shared_ptr<mapped_image_t> find_module(std::string_view name);
	[[nodiscard]] std::shared_ptr<mapped_image_t> find_module_from_rip(emulator_t::address_type rip);
