...
std::shared_ptr<mapped_image_t> kernel::find_module(const std::string_view name)
{
...
std::shared_ptr<mapped_image_t> kernel::find_module_from_rip(const emulator_t::address_type rip)
{
	const auto it = std::ranges::find_if(module_entries,
		[rip](const std::shared_ptr<mapped_image_t>& image) -> bool
		{
